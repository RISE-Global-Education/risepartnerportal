          ...
          <tbody className="divide-y divide-gray-100">
            {filteredMatched.length === 0 ? (
              ...
              filteredMatched.map((r) => (
                <tr key={r.uid} className={`transition-colors ${r.hasNotes ? "hover:bg-gray-50" : "bg-red-50 hover:bg-red-100"}`}>
                  <td className="px-4 py-3 font-mono text-xs text-rise-brown">{r.applicantId}</td>
                  <td className={`px-4 py-3 font-medium ${r.hasNotes ? "text-rise-black" : "text-red-800"}`}>{r.studentName}</td>
