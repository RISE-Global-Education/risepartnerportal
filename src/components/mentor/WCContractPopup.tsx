...
const RATE_REGEX = /^\d{2,4}\s+(USD|GBP|INR)$/;
...
export default function WCContractPopup({ initialName = "", initialEmail = "", onClose }: Props) {
  ...
  async function handleSendClick() {
    if (!checked) {
      ...
      setSendError("");
      try {
        const res = await fetch(`/api/wc-contract?email=${encodeURIComponent(email.trim())}`);
  ...
  async function send() {
    ...
    setSendError("");
    try {
      const res = await fetch("/api/wc-contract", {
        method: "POST",
